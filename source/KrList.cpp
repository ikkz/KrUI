...
namespace KrUI
{
	...
	void KrList::SetPosition(int position)
	{
		...
		else if (position > m_TotalHeight - GetHeight())
		{
	...
	LRESULT KrList::HandleMessage(UINT Message, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_MOUSEMOVE:
			...
			if (m_MouseDownOnScrollBarPos != -1)
			{
				...
				//解以上方程得：
				SetPosition((m_pKrWindow->m_ptMouse.y - m_MouseDownOnScrollBarPos - GetY())*
	...
	void KrList::Update()
	{
		...
		{
			m_ScrollBarRect.Height = (GetHeight()*static_cast<float>(GetHeight()) / m_TotalHeight);
